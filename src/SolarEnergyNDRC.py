...
rec_num = 5356                                                               #########################  need modification  ####################
    ...
    def run(self):
        ...
        for i in self.llist:
            ...
            for j in range(len(resultset_a)):
                ...
                try:
                    LOCK.acquire()
                    sql = "insert into ndrc_solar_energy values(%s,%s,%s,%s)"   ################# need modification of table ####################
...
def test():
    thread1 = MyThread('A',0,25)
    thread2 = MyThread('B',25,50)
