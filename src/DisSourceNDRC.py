...
rec_num = 101                                                               #########################  need modification  ####################
rec_list = [i for i in range(1,rec_num/FORM_DATA['pageSize']+2)]              #################  11 pages ######################
    ...
    def run(self):
        ...
        for i in self.llist:
            ...
            for j in range(len(resultset_a)):
                ...
                try:
                    LOCK.acquire()
                    sql = "insert into ndrc_dis_source values(%s,%s,%s,%s)"   ################# need modification of table ####################
...
def test():
    thread1 = MyThread('A',0,11)
    #thread2 = MyThread('B',5,11)
    #thread3 = MyThread('D',50,75)
    #thread4 = MyThread('E',75,100)
    #thread5 = MyThread('F',100,125)
    #thread6 = MyThread('G',125,150)
    #thread7 = MyThread('H',150,175)
    #thread8 = MyThread('I',175,200)
    #thread9 = MyThread('J',200,225)
    #thread10 = MyThread('K',225,250)
    #thread11 = MyThread('L',250,268)
